...
export default function GroupSessionsList() {
  ...
  function getSessionState(session, myId) {
    const isRegistered = session.registeredMentees?.some(
      (m) => m.id === myId
    );
    ...
    <div className="space-y-6">
      {sessions.map((session) => {
        ...
        const labelConfig = {
          ...
          LIVE: {
            text: "🔴 Live Now",
            classes: "bg-red-500/10 text-red-400 border-red-500/20 animate-pulse",
          },
          COMPLETED: {
            text: "Ended",
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
              ...
              {/* Status Badge */}
              <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border shrink-0 w-fit sm:self-start ${currentLabel.classes}`}>
                {currentLabel.text}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm border-t border-slate-800/60 pt-4 mb-4">
              ...
              <div className="flex items-center gap-2 text-slate-400 sm:justify-end">
                ...
                <span className="text-slate-200 font-medium">
                  {session.registeredMentees?.length || 0} / {session.maxParticipants || "∞"} slots filled
                </span>
              </div>
            </div>
    ...
    <span className="text-slate-100">{mentorModal.mentor.user.email}</span>
            ...
            <div className="flex items-center justify-end pt-2">
              {state === "NOT_REGISTERED" && (
                ...
                >
                  {rsvpLoading === session.id ? "Registering..." : "Reserve Spot"}
                </button>
              )}
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl transform transition-all duration-300 scale-100">
            ...
            <p className="text-slate-300 mb-6 text-sm leading-relaxed">
              Are you sure you want to register for <span className="text-blue-400 font-semibold">{confirmSession.topic}</span>? This will secure your participant slot and register your account profile.
            </p>
